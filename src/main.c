...
int main(int argc, char const *argv[])
{
    FILE *fp;
    printf("PASS LINE 7\n");
    log = fopen("logs/user_logs.txt", "w");
    ...
    {
        printf("PASS LINE 17\n");
        if (solve_equation_yes_no(&response, log) == 1)
        {
        }
        else
        {
...
int memory_ok(Line *linein, FILE *log)
{
    ...
    {
        fprintf(log, "Systems failure no memory for malloc(%d)\n", NLINE);
        return 0;
    }
    linein->len = 0;
    linein->code = -1;
    linein->str = '\0';
    fprintf(log, "Systems is ok for memory of malloc(%d)\n", NLINE);
    return 1;
}

int solve_equation_yes_no(char *response, FILE *log)
{
    printf("Do you want to solve a quadratic eqauation Ax^2 + Bx + B = 0 ? (Y/N):");
    if (fgets(response, 2, stdin) == NULL)
    {
        fprintf(log, "Input error: can not read from stdin\n");
        return -1;
    }
